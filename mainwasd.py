...
def paper_hybrid_loss(pred, target):
    # 1. 权重调整：强化管状结构与方向一致性损失，适配OAGF思想
    alpha_focal = 0.35  # Focal损失权重（略降，为新增损失腾空间）
    alpha_ce = 0.2      # BCE损失权重（保持）
    alpha_dice = 0.3    # Dice损失权重（略降）
    alpha_consistency = 0.05  # 血管区域一致性损失（保持）
    alpha_tubular = 0.07      # 新增：管状结构损失权重
    alpha_gradient = 0.03     # 新增：方向梯度一致性损失权重
    gamma = 1.5  # Focal难例加权系数（保持）
    
    # BCE Loss（保持不变）
    ce_loss = torch.nn.BCEWithLogitsLoss()(pred, target)
    pred_sigmoid = torch.sigmoid(pred)
    
    # Focal Loss（保持标准公式，适配难例）
    p_t = torch.where(target == 1, pred_sigmoid, 1 - pred_sigmoid)
    focal_weight = (1 - p_t) ** gamma
    focal_loss = -alpha_focal * focal_weight * torch.log(p_t + 1e-8)
    focal_loss = focal_loss.mean()
    
    # Dice Loss（保持平滑项，强化前景重叠）
    ...
    def vessel_region_consistency_loss(pred_s, mask, temp=0.1):
        ...
        pred_smoothed = pred_smoothed * vessel_mask
    ...
    # -------------------------- 新增1：管状结构损失（适配OAGF管状滤波思想） --------------------------
    def tubular_structure_loss(pred_s, mask, kernel_size=3):
        """
        模仿OAGF的Hessian管状检测，惩罚非管状的血管预测（如墨水状噪声）
        """
        vessel_mask = (mask > 0.5).float()
        if vessel_mask.sum() == 0:
            return torch.tensor(0.0, device=pred_s.device)
        
        # 用拉普拉斯核模拟Hessian矩阵，突出管状结构（血管为管状，噪声为弥散状）
        laplacian_kernel = torch.tensor([[[[0, 1, 0], [1, -4, 1], [0, 1, 0]]]], dtype=torch.float32).to(pred_s.device)
        # 计算预测图的管状响应（管状区域响应弱，噪声区域响应强）
        tubular_resp = torch.abs(F.conv2d(pred_s, laplacian_kernel, padding=1))
        # 仅在血管区域惩罚强响应（即非管状结构）
        tubular_loss = (tubular_resp * vessel_mask).mean()
        return tubular_loss
    tubular_loss = tubular_structure_loss(pred_sigmoid, target)
    
    # -------------------------- 新增2：方向梯度一致性损失（适配OAGF多方向梯度思想） --------------------------
    def direction_gradient_consistency_loss(pred_s, mask):
        """
        模仿OAGF的4/8方向梯度，强化血管沿方向的连续性，避免断裂
        """
        vessel_mask = (mask > 0.5).float()
        if vessel_mask.sum() == 0:
            return torch.tensor(0.0, device=pred_s.device)
        
        # 定义4个对角线方向梯度核（东南、西北、东北、西南，适配你的模型设计）
        kernels = torch.tensor([
            [[[0, 0, 0], [0, -1, 0], [0, 0, 1]]],  # 东南(SE)
            [[[1, 0, 0], [0, -1, 0], [0, 0, 0]]],  # 西北(NW)
            [[[0, 0, 1], [0, -1, 0], [0, 0, 0]]],  # 东北(NE)
            [[[0, 0, 0], [0, -1, 0], [1, 0, 0]]]   # 西南(SW)
        ], dtype=torch.float32).to(pred_s.device)
        
        # 计算4方向梯度响应
        grad_feats = torch.abs(F.conv2d(pred_s, kernels, padding=1))  # [B,4,H,W]
        # 血管区域内的梯度应连续（4方向梯度差异小）
        grad_mean = grad_feats.mean(dim=1, keepdim=True)  # [B,1,H,W]
        grad_consistency = torch.mean((grad_feats - grad_mean) ** 2 * vessel_mask.expand_as(grad_feats))
        return grad_consistency
    gradient_loss = direction_gradient_consistency_loss(pred_sigmoid, target)
    
    # 总Loss：权重和=1.0，保持尺度稳定
    total_loss = (alpha_focal * focal_loss + 
                  alpha_ce * ce_loss + 
                  alpha_dice * dice_loss + 
                  alpha_consistency * consistency_loss + 
                  alpha_tubular * tubular_loss + 
                  alpha_gradient * gradient_loss)
    return total_loss
